...
class ARCameraActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var button1 = this.findViewById<Button>(R.id.buttonNext)
        button1.setOnClickListener {
            Log.d("TAG", anchor.pose.toString())
        }
        arFragment =
            (supportFragmentManager.findFragmentById(R.id.scene_form_fragment) as CustomArFragment).apply {
    ...
    private fun displayWidget(arFragment: ArFragment, anchor: Anchor) {
        ViewRenderable.builder().setView(this, R.layout.ar_info_display_widget)
            ...
            .thenAccept { viewRenderable ->
                addWidgetToScene(arFragment, anchor, viewRenderable)
            }
    }

    private fun addWidgetToScene(
        arFragment: ArFragment,
        anchor: Anchor,
        viewRenderable: ViewRenderable
    ) {
        var anchorNode: AnchorNode = AnchorNode(anchor)
        //anchorNode.worldPosition = Vector3(anchor.pose.tx(),anchor.pose.ty(),anchor.pose.tz())
